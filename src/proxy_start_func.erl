...
-export_type([
              start_func/0,
              start_func_spawn/0, start_func_start/0,
...
-type start_func() :: start_func_spawn() | start_func_start(). % FIXME: 処理の内容を把握してから名前を検討.

-type start_func_spawn() :: {spawn, module(), function(), args(), spawn_options()}
                          | {spawn, fun(), spawn_options()}.
-type start_func_start() :: {start, module(), function(), args(), time(), start_options()}.
...
-spec make_spawn_func(module(), function(), args()) -> start_func_spawn().
make_spawn_func(Module, Function, Args) ->
    make_spawn_func(Module, Function, Args, []).

-spec make_spawn_func(module(), function(), args(), spawn_options()) -> start_func_spawn().
make_spawn_func(Module, Function, Args, Options) ->
    {spawn, Module, Function, Args, Options}.

-spec make_spawn_func(fun()) -> start_func_spawn().
make_spawn_func(Fun) ->
    make_spawn_func(Fun, []).

-spec make_spawn_func(fun(), spawn_options()) -> start_func_spawn().
make_spawn_func(Fun, Options) ->
    {spawn, Fun, Options}.

%% @doc サーバを実行するための関数セットを作る.
-spec make_start_func(module(), function(), args()) -> start_func_start().
make_start_func(Module, Function, Args) ->
    make_start_func(Module, Function, Args, 5000).

-spec make_start_func(module(), function(), args(), time()) -> start_func_start().
make_start_func(Module, Function, Args, Time) ->
    make_start_func(Module, Function, Args, Time, []).

-spec make_start_func(module(), function(), args(), time(), start_options()) -> start_func_start().
make_start_func(Module, Function, Args, Time, Options) ->
    {start, Module, Function, Args, Time, Options}.

%% @doc 引数を取得する.
-spec get_args(start_func()) -> {ok, args()} | error.
get_args({spawn, _, _, Args, _})    -> {ok, Args};
get_args({start, _, _, Args, _, _}) -> {ok, Args};
get_args(_)                         -> error.

%% @doc 引数をセットする.
-spec set_args(args(), start_func()) -> start_func().
...
%% @doc 関数を開始する.
-spec start_link(start_func_spawn()) -> {ok, pid() | {pid(), reference()}}
    ;           (start_func_start()) -> {ok, pid()} | term().
